...
- Detects timestamp gaps and assigns `segment_id` so feature windows never bridge discontinuities.
- Builds configurable time windows (default **1 hour**, stepped every **30 minutes**).
...
- Uses a **chronological train/test split** to reduce temporal leakage.
...
MetroPT-3 contains multivariate telemetry collected from an Air Production Unit (APU) in an operational metro train. The source dataset includes pressure, temperature, motor-current and digital-control signals sampled at approximately 1 Hz.
...
The raw CSV is intentionally **not committed**. Download `MetroPT3(AirCompressor).csv` from UCI and place it at:
...
The default validation/window settings live in `src/metropt3/config.py` and can be changed without rewriting the pipeline.

## Why segmentation matters

MetroPT is a long operational time series containing gaps and maintenance periods. Treating the entire file as one continuous stream can create feature windows that combine unrelated points across a data gap.

`validate_and_segment()` therefore starts a new segment when the timestamp gap exceeds the configured threshold (default: 5 seconds). `build_windows()` then groups by `segment_id`, so no window can cross a segment boundary.
...
The built-in scenarios are explicitly synthetic. If `artifacts/model.joblib` exists, the dashboard can show the trained classifier's failure-risk probability for the latest feature window. If no trained model artifact exists, it shows a clearly labeled **heuristic demo health-risk indicator**, not an ML prediction.

That distinction keeps the hosted/user-facing demo useful without presenting fabricated model performance.
...
- The default classifier is a strong, interpretable baseline pipeline component, not evidence that Random Forest is optimal for MetroPT.
- A 12-hour horizon is a modeling choice and should be evaluated against operational maintenance requirements.
...
- Chronological splitting reduces leakage risk but does not replace more rigorous rolling-origin/backtesting studies.
...
- Dedicated Streamlit Community Cloud demo branch once a verified public-demo artifact strategy is chosen.
...
This repository's code implements the validation, segmentation, windowing, feature engineering, labeling, evaluation and visualization pipeline around the public dataset; it does not claim ownership of the MetroPT-3 data.
